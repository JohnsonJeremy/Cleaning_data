run_analysis=function(){
     ...
     write.table(accel_data, file="tidy_HAR_dataset.txt", row.names = FALSE)

    
     ## Use the aggregate function to subset the tidy data by subject and activity
     # and calculate the mean for each measured value
     means_frame<-aggregate(x=accel_data[4:9], by=list(accel_data$subject, 
                              accel_data$activity), FUN=mean, na.rm=TRUE)
     ...
     write.table(means_frame, file="tidy_HAR_means.txt", row.names = FALSE)
